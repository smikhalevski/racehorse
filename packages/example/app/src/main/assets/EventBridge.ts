...
function createEventBridge<I, O = unknown>(): EventBridge<I, O> {
  ...
  const pendingRequestsMap = new Map<number, { resolve(value: any): void, reject(reason): void }>();
  ...
  const request = (javaClass: string, event: any): Promise<any> => {
  ...
  const subscribe = (javaClass: string, listener: (event: any) => void): () => void => {
