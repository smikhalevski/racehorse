...
import { Connection, createEventBridge } from '../main/index.js';

let connectionMock: Connection | undefined;
let provideConnection: () => void;
let connectionProviderMock = vi.fn(() => connectionMock);
...
describe('requestAsync', () => {
  ...
  test('rejects if an exception is published', async () => {
    ...
    connectionMock!.post = vi.fn().mockImplementationOnce(() => {
      setTimeout(
        () =>
    ...
    await expect(eventBridge.requestAsync({ type: 'aaa' })).rejects.toEqual(expect.any(Error));
  });

  test('multiple bridges can coexist', async () => {
    provideConnection();

    connectionMock!.post = vi
      .fn()
...
describe('request', () => {
  ...
  test('posts a sync request', () => {
    ...
    connectionMock!.post = vi.fn().mockImplementationOnce(() => '{"type":"bbb"}');
  ...
  test('establishes connection only once', () => {
    ...
    connectionMock!.post = vi.fn().mockImplementation(() => '{"type":"bbb"}');
  ...
  test('throws if an exception is returned', () => {
    ...
    expect(() => eventBridge.request({ type: 'aaa' })).toThrow(expect.any(Error));
  });
});
